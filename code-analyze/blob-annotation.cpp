...
void Blob<Dtype>::Reshape(const vector<int>& shape) {
  ...
  count_ = 1;
  shape_.resize(shape.size()); // 重新定义vector shape_的size
  if (!shape_data_ || shape_data_->size() < shape.size() * sizeof(int)) {
    shape_data_.reset(new SyncedMemory(shape.size() * sizeof(int)));
  }
  int* shape_data = static_cast<int*>(shape_data_->mutable_cpu_data());
  for (int i = 0; i < shape.size(); ++i) {
    CHECK_GE(shape[i], 0); // 保证shape每个元素为正数
    if (count_ != 0) {
  ...
  if (count_ > capacity_) { // blob存储元素个数count_大于当前元素capacity_，所以需要重新分配内存空间
    capacity_ = count_;
    // 构造了SyncedMemory对象，并未真正分配内存和显存, 真正分配是在第一次访问数据时
    data_.reset(new SyncedMemory(capacity_ * sizeof(Dtype)));
    diff_.reset(new SyncedMemory(capacity_ * sizeof(Dtype)));
  }
}

// 用protobuf传递来的dim 解析, 并对shape_ 进行reshape
template <typename Dtype>
...
// 用构造函数的重载方法定义2个构造函数，以便提供不同的初始化方法
template <typename Dtype>
